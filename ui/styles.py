...
/* ---------- 8c. Tríadas pairwise — tarjetas grandes clickeables ---------- */
/* Todos los botones dentro del contenedor pairwise_row son las tarjetas */
...
.st-key-pairwise_row .stButton > button:hover {
    transform: translate(2px, 2px) !important;
    box-shadow: 3px 3px 0px var(--ink) !important;
}
.st-key-pairwise_row .stButton > button[kind="primary"] {
...
/* ---------- 8d. Tarjetas de selección de dominio ---------- */
[class*="st-key-domcard_"] {
    border: 2px solid var(--ink);
    border-radius: 12px;
...
/* --- 17b. Área de pregunta con altura mínima fija ---
   Reserva espacio equivalente a ~2 líneas de texto para que
   los botones de navegación no suban/bajen según la longitud
   de la pregunta. */
.st-key-quiz_question {
    min-height: 4.5rem;
    display: flex;
    align-items: flex-start;
}

/* --- 17c. Botonera inferior centrada (Salir / Anterior / Siguiente) ---
   Las columnas de Streamlit se convierten en auto-width para que
   cada botón tenga el tamaño de su etiqueta, igual que las opciones. */
.st-key-quiz_actions [data-testid="stHorizontalBlock"] {
    justify-content: center !important;
    gap: 0.8rem !important;
}
.st-key-quiz_actions [data-testid="stColumn"] {
    flex: 0 0 auto !important;
    min-width: 0 !important;
    width: auto !important;
}
.st-key-quiz_actions .stButton > button {
    min-height: 46px;
    padding: 0.45rem 1.2rem !important;
}
/* Salir (✕): rojo de alerta. */
...
/* Anterior (←): neutro, idéntico al secundario base. */
...
/* --- 17d. Zero-Scroll 100dvh — SOLO mobile (en desktop deja fluir) --- */
@media (max-width: 768px) {
...
@media (min-width: 1200px) {
    ...
    /* Contenedor principal: máx 1200px centrado para no estirar el texto
