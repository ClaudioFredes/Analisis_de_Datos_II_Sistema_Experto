...
# -----------------------------------------------------------------
# Fase 3 — Selección de pathway y scoring con boosts directos
# -----------------------------------------------------------------
def seleccionar_pathway(pathways: list[dict], vector: dict[str, float]) -> dict:
    """Elige el pathway cuyo dims_trigger contiene la dimensión dominante del vector."""
    top_dim = max(DIMENSIONES_RIASEC, key=lambda d: vector.get(d, 0.0))
    for p in pathways:
        if top_dim in p.get("dims_trigger", []):
            return p
    return pathways[0]  # fallback: primer pathway


def aplicar_boosts_f3(
    ranking: list[dict],
    ...
    result = []
    for c in ranking:
        boost = boost_map.get(c["id"], 0.0)
        score_final = c["score"] + boost
        result.append({
            **c,
            "score":        score_final,
            "afinidad_pct": int(round(max(0.0, score_final) * 100)),
            "boost_f3":     round(boost, 3),
        })
    result.sort(key=lambda x: (-x["score"], -x.get("pearson", 0), x["id"]))
    return result


# -----------------------------------------------------------------
# Scoring pairwise (comparación forzada por tríadas)
# -----------------------------------------------------------------
def calcular_vector_pairwise(
    elecciones: dict[str, str],
    triadas: list[dict],
) -> dict[str, float]:
    """Convierte elecciones de tríadas en un vector RIASEC 1-5.

    Para cada dimensión:
        wins        = veces que el usuario eligió una opción de esa dimensión
        appearances = veces que esa dimensión apareció en alguna tríada
        score       = 1 + (wins / appearances) * 4  →  rango [1.0, 5.0]

    Una dimensión que nunca apareció recibe 3.0 (neutro).
    Compatible con ranking_carreras() que espera el mismo formato de vector.
    """
    wins: dict[str, int] = {d: 0 for d in DIMENSIONES_RIASEC}
    apps: dict[str, int] = {d: 0 for d in DIMENSIONES_RIASEC}

    triada_map = {t["id"]: t for t in triadas}

    for triada_id, opcion_elegida_id in elecciones.items():
        triada = triada_map.get(triada_id)
        if not triada:
            continue
        for opcion in triada["opciones"]:
            dim = opcion["dimension"]
            if dim in apps:
                apps[dim] += 1
        for opcion in triada["opciones"]:
            if opcion["id"] == opcion_elegida_id:
                wins[opcion["dimension"]] += 1
                break
    ...
    for dim in DIMENSIONES_RIASEC:
        if apps[dim] == 0:
            ...
            vector[dim] = round(1.0 + (wins[dim] / apps[dim]) * 4.0, 2)
    return vector
