...
def etiquetas_vetadas(
    ...
    Cada filtro puede definir su propio umbral de veto con el campo
    opcional `veto_umbral`. Si no lo tiene, se usa UMBRAL_VETO global.
    Esto permite que filtros con alta ambigüedad semántica (ej. ciencias
    naturales) veten con 3 ("me da igual"), ya que neutral implica que
    ese mundo no es el de la persona.

    Args:
        respuestas_filtro: dict {id_filtro: puntaje_1_a_5}.
        filtros: lista de filtros (cada uno con 'id', 'etiqueta' y
                 opcionalmente 'veto_umbral').

    Returns:
        Set de etiquetas con respuesta <= umbral. Vacío si el
        usuario no vetó nada (todas las carreras siguen en juego).
    """
    vetadas: set[str] = set()
    for f in filtros:
        respuesta = respuestas_filtro.get(f["id"])
        umbral = f.get("veto_umbral", UMBRAL_VETO)
        if respuesta is not None and respuesta <= umbral:
